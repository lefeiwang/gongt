...
// GetIndexSize returns number of the indexed vectors
//	indexSize := gongt.GetIndexSize()
func GetIndexSize() int {
	return ngt.GetIndexSize()
}

// GetIndexSize returns number of the indexed vectors
//	indexSize := gongt.Get().GetIndexSize()
//	indexSize := gongt.New("index path").GetIndexSize()
func (n NGT) GetIndexSize() int {
	ebuf := C.ngt_create_error_object()
	defer C.ngt_destroy_error_object(ebuf)

	return int(C.ngt_get_number_of_indexed_objects(n.index, ebuf))
}
